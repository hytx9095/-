...
import com.wrbi.springbootinit.bizmq.BiMessageProducer;
...
import com.wrbi.springbootinit.model.enums.ChartStatusEnum;
import com.wrbi.springbootinit.manager.AiManager;
import com.wrbi.springbootinit.model.dto.chart.*;
import com.wrbi.springbootinit.model.entity.Integral;
import com.wrbi.springbootinit.model.vo.BiResponse;
...
import com.wrbi.springbootinit.manager.RedisLimiterManager;
import com.wrbi.springbootinit.model.entity.Chart;
import com.wrbi.springbootinit.model.entity.User;
import com.wrbi.springbootinit.utils.ExcelUtils;
...
public class ChartController {
     ...
     */
    @PostMapping("/delete")
    public BaseResponse<Boolean> deleteChart(@PathParam("chartId") long chartId, HttpServletRequest request) {
...
//        String result = aiManager.sendMesToAIUseXingHuo(userInput.toString());
        ...
        //扣减积分
        integralService.genChartByIntegral(UserContext.getUserId());
     ...
     * 智能分析（异步）重新生成
     *
     * @param genChartByAiRetryRequest
     * @param request
     * @return
     */
    @PostMapping("/gen/async/retry")
    public BaseResponse<BiResponse> genChartByAiAsyncRetry(GenChartByAiRetryRequest genChartByAiRetryRequest,
                                                           HttpServletRequest request) {
        // 判断积分是否充足
        if (!checkIntegralEnough()) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "积分不足");
        }
        Chart chart = chartService.getById(genChartByAiRetryRequest.getChartId());
    ...
    public BaseResponse<BiResponse> genChartByAiAsyncMqRetry(GenChartByAiRetryRequest genChartByAiRetryRequest,
                                                             HttpServletRequest request) {
        // 判断积分是否充足
        if (!checkIntegralEnough()) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "积分不足");
        ...
        redisLimiterManager.doRateLimit("genChartByAi_" + loginUser.getId());
        // 发送消息
        biMessageProducer.sendMessage(String.valueOf(genChartByAiRetryRequest.getChartId()));
        BiResponse biResponse = new BiResponse();
        biResponse.setChartId(genChartByAiRetryRequest.getChartId());
        return ResultUtils.success(biResponse);
    }

    private boolean checkIntegralEnough() {
        // 判断积分是否充足
        long userId = UserContext.getUserId();
        Integral integral = integralService.lambdaQuery().eq(Integral::getUserId, userId).one();
        if (integral == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR);
        }
        if (integral.getTotalIntegral() + IntegralConstant.GEN_CHART_INTEGRAL < 0) {
            return false;
        }
        return true;
    }
    ...
    private void genByAsync(Chart chart, StringBuilder userInput) {
...
//            String genChart = splits[1].trim();
            String genChart = checkGenChart(splits[1].trim());
            String genResult = splits[2].trim();
            Chart updateChartResult = new Chart();
    ...
    public String checkGenChart(String genChart) {
        //将单引号改为双引号
